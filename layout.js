function getStyle(ele) {
    ...
    for (var prop in ele.computeStyle) {
        ele.style[prop] = ele.computeStyle[prop].value;
...
function layout(ele) {
    ...
    if (!ele.computeStyle || JSON.stringify(ele.computeStyle) == "{}") {
        return;
    }
    ...
    var items = ele.children.filter(e => e.type === 'element');
    items.sort((a, b) => {
    ...
    if (!style[mainSize]) {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            if (itemStyle[mainSzie] !== null || itemStyle[mainSzie] !== (void 0)) {
                eleStyle[mainSize] = eleStyle[mainSize] + itemStyle[mainSzie];
            }
        }
        isAutoMainSize = true
    }
    ...
    if (mainSpace < 0) {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            itemStyle[mainSize] = ite < nStyle[mainSize] * scale;
        ...
        flexLines.forEach(function (items) {
            ...
            if (flexTotal > 0) {
                ...
                for (var i = 0; i < items.length; i++) {
                    ...
                    var itemStyle = getStyle(item);
                    if (itemStyle.flex) {
                        itemStyle[fnainSize] = (mainSpace / flexTotal) * itemStyle.flex;
                    }
                    itemStyle[mainStart] = currentMain
                    itemStyle[mainEnd] = itemStyle[mainStart] + mainSign * itemStyle[mainSize]
                    currentMain = itemStyle[mainEnd];
